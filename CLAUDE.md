...
- 持久化：账目类核心数据（交易、账户、分组、账本、分类、标签、月度/分类预算）**只**存 SQLite，由 [lib/data/](lib/data/) 的 `AppDatabase`（建表 + 版本迁移）与 `LedgerRepository`（接口，整表覆盖式读写）承载，生产实现 `SqliteLedgerRepository`。controller 内存列表仍是读取源，写入经 `_persistX` 落库。`VeriFinController.create()`（唯一构造入口，异步）从库载入；全新数据库首启动播种默认账本/账户/分组/分类（标签默认为空）。交易与标签为多对多，交易表 `tag_ids` 列存 JSON 数组，不建关联表；标签全局共享不分账本。没有 KV↔SQLite 迁移，也没有 KV 回退——账目类只认库。偏好类小数据（主题、触感、面板配置、资产排序/折叠、封面、活动账本、个人资料等）仍以 JSON 存入 `LocalKeyValueStore`，键形如 `verifin.theme.v1`。修改 `lib/data/` 表结构须提升 `AppDatabase.schemaVersion` 并在 `_onUpgrade` 写迁移。测试注入内存实现 `InMemoryLedgerRepository`（见 `test/support/`）。
