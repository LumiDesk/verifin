...
- [ ] **7.2 支付平台账单导入（等示例数据）**：扩展现有 CSV 导入（[transaction_import.dart](lib/app/backup/transaction_import.dart)，已支持钱迹/随手记/自有模板表头识别），增加微信、支付宝及其他支付/记账软件的账单格式识别，并补一份「如何导出账单」的用户引导。**约束：每种来源的表头/字段解析必须基于用户提供的真实导出样例，不得凭空编造格式**；用户逐个提供样例后逐个实现。合规、基础设施已就绪。
