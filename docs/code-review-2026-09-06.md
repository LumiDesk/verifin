...
请直接在下表“回复”列填写。建议每项至少注明：**结论（确认 / 不确认 / 需补证据）**、**处理方式（修复 / 优化 / 保留）**、**优先级**，以及必要的补充场景。处理完成后可把“状态”改为“已决定”或“待复核”。

| 编号 | 对应问题 | 状态 | 回复 |
| --- | --- | --- | --- |
| R-01 | 工作树中的 `pubspec.lock` 已损坏 | 待回复 | <!-- 在这里填写你的判断与处理决定 --> |
| R-02 | 发布签名私钥进入 Git | 待回复 | <!-- 在这里填写你的判断与处理决定 --> |
| R-03 | “代还”转账无法在交易详情编辑 | 待回复 | <!-- 在这里填写你的判断与处理决定 --> |
| R-04 | 发布版首页最近交易由 5 条变为 3 条 | 待回复 | <!-- 在这里填写你的判断与处理决定 --> |
| R-05 | 悬空账户回退显示为首个账户 | 待回复 | <!-- 在这里填写你的判断与处理决定 --> |
| R-06 | 悬空分类静默改成首个分类 | 待回复 | <!-- 在这里填写你的判断与处理决定 --> |
| R-07 | 小组件跨周期显示旧预算 / 跨日忽略未来交易 | 待回复 | <!-- 在这里填写你的判断与处理决定 --> |
| R-08 | Android 系统入口硬编码中文 | 待回复 | <!-- 在这里填写你的判断与处理决定 --> |
| R-09 | 备份归档丢失附件 MIME 类型 | 待回复 | <!-- 在这里填写你的判断与处理决定 --> |
| R-10 | 快速记账占位高度不遵循字体缩放 | 待回复 | <!-- 在这里填写你的判断与处理决定 --> |
| R-11 | 备份读取和解包缺少大小限制 | 待回复 | <!-- 在这里填写你的判断与处理决定 --> |
| R-12 | 自动备份最外层异常静默吞掉 | 待回复 | <!-- 在这里填写你的判断与处理决定 --> |
