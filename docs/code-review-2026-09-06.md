...
> **历史记录**：本文件记录 2026-09-06 当时的问题清单。其中绝大多数 P0/P1 已在后续提交中修复（`pubspec.lock` 损坏、首页最近交易条数、代还转账编辑、悬空账户/分类回退、Android 硬编码中文已部分修复等），**不能当作当前缺陷清单**。唯一仍成立的是「发布签名私钥已入库」，且用户已明确接受现状。当前状态以源码、测试与 `docs/dev/known-limitations.md` 为准。
