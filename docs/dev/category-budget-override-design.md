...
- 在当时的 v1.11 基线中，`BudgetOverviewPage` 的分类预算树被改为只读，没有入口修改或清除旧的单期覆盖；
...
- 点击有子分类行尾的展开/收起箭头：只切换子树，不打开编辑弹窗；
...
1. **交互**：在预算总览中点击分类行主体，管理当前所选月份/周期的分类单期预算；有子分类的行尾箭头仍只负责展开/收起子分类。
2. **零值语义**：保持旧行为，输入 0/选择清除都表示删除单期覆盖并回落默认值，不新增“显式 0 覆盖”。
3. **兼容策略**：不自动把旧的月度分类预算迁成默认预算，所有旧值原样保留，由用户按需修改或恢复默认。

实现与本文的数据和交互口径一致；v1.17.7（提交 `bcb33de`）对分类预算行做了视觉重做：移除行尾预算数字，展开箭头改为整行右侧垂直居中的旋转控件。总预算与分类预算覆盖 helper 已收口到 `sheets.dart`，并额外修正
...
flutter test test/budget_test.dart    通过（25 tests）
flutter test                          通过（1034 tests，5 skipped）
```
