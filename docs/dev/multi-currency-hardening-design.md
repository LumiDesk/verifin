...
> 状态：**已实现；自动化验收完成，Android release 真机验收待发布后执行**
>
...
- MC-01～MC-14 已按本文完成：外币退款导入直接生成关联条目，历史金额冻结，JPY/KWD
  精度收口，本位币比较、到账日趋势、汇率 trace、周期缺率提示、AI 转账和小组件刷新均已落地；
...
- 已保存交易显式点“重新计算”后才更新；
- 手工汇率后改原币金额保持手工汇率；
- 单笔编辑改账户只更新账户金额，不改变冻结统计金额；交易列表批量改账户仅允许来源账户币种一致且已有实际 `accountAmount` 的同币种搬移，跨币种/无账户/转入冲突整批拒绝并提示逐笔编辑；
- 有退款支出无法改原币，并显示本地化原因；
...
- [x] 外币退款导入不会混用三种金额单位，CSV 重导入余额与净额一致；
- [x] 已保存交易只改日期不会改变任何冻结金额；
- [x] 所有金额展示、输入、零值判断和预算持久化遵循实际币种 minor unit；
- [x] 报销筛选、交易排序和自动识别不再比较不同币种裸数字；
- [x] 退款只在 `settledAt` 改变账户/资产趋势；
- [x] 有关联退款的支出不能进入原币不一致的不可保存状态；
- [x] 导入/恢复在写库前通过统一三层金额与引用校验；
- [x] 汇率生效日期、陈旧状态和周期缺率有明确用户可见反馈；
- [x] 汇率、预算、账户、账本和交易变化能及时刷新桌面小组件；
- [x] AI 转账查询不再把金额展示为 0；
- [x] `dart format .`、`flutter analyze`、`flutter test` 全部通过；
- [ ] Android release APK 完成专项真机验收；
- [x] 相关文档和 `CHANGELOG.md` 与最终实现同步。
