...
**进备份**（随 `exportDataJson` 的 `data` 导出、可跨设备还原）：全部账目数据（账本/交易/账户/分组/分类/标签/附件/周期规则/月度·分类·按日预算）+ 个人资料 + 活动账本 + 主题、触感、资产封面、资产视图模式/折叠/排序、首页面板、看板面板 + **默认付款账户 `default_account`（`Map<bookId,accountId>`）、预算周期起始日 `budget_cycle`、FAB 行为 `fab_action`、金额小数位 `amount_format`、记账自动识别开关 `auto_suggest`、首页概览卡配置 `home_metrics`**。缺少新字段的旧备份按各功能默认值导入；这些偏好与 theme/haptics 等使用同一套“备份内容覆盖本机状态”的语义。
