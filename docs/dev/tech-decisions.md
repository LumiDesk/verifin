...
| 账目落库写入策略 | `saveX` 走**行级差分**（`_incrementalReplace`）：交易/账本/账户/分组/分类/标签/周期规则/汇率各在内存保留「上次落库的行快照」（id→规范化行），只写与快照不同的行（upsert 变化行、delete 消失行）；无变化不写。附件表（大 blob，缓存整行会成倍占内存）与预算表（极小）仍整表覆盖；导入/恢复/重置走 `replaceAllLedgerData` 多表原子整替（整替后重建各表快照基线）。`saveX` 对外语义不变（落库后表内容 == 传入列表） | 原「删全表 + 全量重插」使**单条记账就重写整张 `entries` 表**，写放大随交易量线性增长、磨损存储；`entries` 无界增长是主因。差分把语义等价的写降到「只写变化行」。快照从 `loadX`/`replaceAllLedgerData` 的规范化行 seed，保证重启后首次删除也有正确基线 |
| 交易聚合编辑与落库 | 交易详情把原交易、关联退款和附件作为一个页面草稿；Controller 的 `saveEntryAggregateDraft` 校验三层金额与引用，并从已到账退款重新派生 `refundedBaseAmount`。用户选择「记住汇率」时，同日汇率也和交易、附件一起交给 Repository 的 `saveEntryAggregate`，在同一个 SQLite 事务中提交；成功后才更新内存并重建 entries/exchange_rates 差分快照 | 退款、附件或用于本次交易的汇率若在父交易保存前单独落库，会形成多套提交边界；聚合草稿让放弃操作零副作用，事务避免只保存一部分，提交时重算派生值则从数据层堵住旧快照覆盖（issue #31） |
...
| 桌面小组件 | 原生 `AppWidgetProvider` + `RemoteViews`，数据经现有 `verifin/app` MethodChannel 写入原生 SharedPreferences，不引入 `home_widget` 依赖；Flutter 侧推送已按本位币格式化的今日支出、预算和资产字符串，资产缺率时推送本地化占位而不是部分总额 | 点击快速记账直接复用已有 `ACTION_QUICK_ENTRY` 机制；原生层只负责持久化和渲染已明确币种语义的字符串，不重复实现财务换算 |
...
**进备份**（随 JSON v2 `exportDataJson` 的 `data` 导出、可跨设备还原）：全部账目数据（账本及本位币确认状态、交易三层金额与转换来源、账户币种、分组、分类、标签、附件、周期规则及汇率策略、用户维护/明确选择保存的汇率表、月度·分类·按日预算）+ 个人资料 + 活动账本 + 主题、触感、资产封面、资产视图模式/排序、资产折叠历史兼容字段（普通浏览页已不再写入）、首页面板、看板面板 + **默认付款账户 `default_account`（`Map<bookId,accountId>`）、预算周期起始日 `budget_cycle`、FAB 行为 `fab_action`、金额小数风格 `amount_format`、记账自动识别开关 `auto_suggest`、首页概览卡配置 `home_metrics`**。v1/缺字段备份按 CNY 原值重解释并标为待确认，旧 `amountForceTwoDecimals` 映射到新显示风格；货币静态目录属于应用代码，不进备份。这些偏好与 theme/haptics 等使用同一套“备份内容覆盖本机状态”的语义。
