...
- [ ] 旧数据库升级后所有现有人民币数值与余额不变。
- [ ] 新账本/账户可以选择支持的法币，使用后按规则锁定。
- [ ] 支出/收入正确保存原币、账户金额和冻结本位币金额。
- [ ] 跨币转账两端余额按真实金额变化，手续费只扣转出账户。
- [ ] 跨币退款支持待到账、跨账户和实际到账金额，净额不为负。
- [ ] 汇率完全离线、可按日手工维护、方向全局一致。
- [ ] 修改/删除汇率不改变历史收支；资产估值按目标日有效汇率。
- [ ] 缺率不猜 1:1、不静默丢数据、不显示部分总资产。
- [ ] 预算、报表、首页、AI 汇总统一使用账本本位币。
- [ ] 账户和交易列表保留原币信息，混币页面无歧义符号。
- [ ] 周期规则缺率时可见、可恢复、不会重复或跳过到期日。
- [ ] 第三方导入和本应用 CSV 保留币种，预览可解决缺率。
- [ ] v1/v2 备份、zip/加密、样例备份和冷启动往返通过。
- [ ] 中英文、无障碍、通知和桌面小组件币种正确。
- [ ] migration/model/repository/controller/widget/AI/导入/备份测试通过。
