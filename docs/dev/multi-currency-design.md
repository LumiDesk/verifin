...
- 交易列表批量改账户只允许原账户币种一致、每笔已有实际 `accountAmount` 且目标不与转入账户冲突的同币种搬移；跨币种和无账户交易没有足够信息推导新账户实际金额，明确要求逐笔编辑，不猜汇率也不重算冻结本位币。
...
- 页面无“刷新”或联网状态；页头问号通过只读弹窗说明“汇率保存在本机，由你维护”及历史交易冻结口径，不常驻说明卡片。
...
> **2026-09-10 更新**：本节第 2 条「Header/卡片标题通过 `MoneyUnitLabel` 标注币种」与第 5 条「并在卡片或页头标明单位」当时是按「单位始终显示」写的，第 1197 行验收清单里的「聚合卡片统一标注单位」同理。此后加入了「单币种隐藏单位」偏好，单币种账本下单位整体不显示：卡片角标 `MoneyUnitLabel` 整块不渲染、调用方为它单独加的间距要一起收掉，页头副标题必须走 `currencyUnitSubtitle`（无上下文时整条省略）。当前口径见 [统一设计规范](../design-system.md) 与 [组件清单](components.md)；本节其余内容仍是多币种的设计记录。
