...
正式入口为 `lib/app/feedback.dart`。当前阶段已安装根级 Host，但尚未替换历史
`SnackBar` 调用；迁移必须按本文分类处理，禁止机械全局替换。
...
当前历史调用以裸 `SnackBar` 为主，按以下批次迁移：

1. 根退出与全局持久化失败，先验证根级 Host 和错误去重。
2. 设置、复制、提醒、小组件等简单成功/说明反馈。
3. 分类、账本、面板、交易批量操作等限制与结果反馈。
4. 数据管理中的备份、导入、导出、WebDAV 状态机。
5. 删除撤销；必须先补领域层恢复契约和回归测试。

每批迁移后删除对应裸 `SnackBar`，运行该领域测试、`flutter analyze` 和全量
`flutter test`。在全部调用迁移完成前，旧 `ScaffoldMessenger` 仍可能存在；新代码
不得再增加裸 `SnackBar`。
