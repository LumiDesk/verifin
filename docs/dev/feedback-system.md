...
退出”和全局持久化失败；简单反馈批次已继续迁移，仍有 52 处历史 `SnackBar`。后续
必须按本文分类处理，禁止
机械全局替换。
...
2. （进行中）设置、应用锁、日志复制、截图能力、还款、提醒、小组件等简单反馈已迁移；账户详情复制随账户批次处理。
3. 分类、账本、面板、交易批量操作等限制与结果反馈。
4. 数据管理中的备份、导入、导出、WebDAV 状态机。
5. 删除撤销；必须先补领域层恢复契约和回归测试。

每批迁移后删除对应裸 `SnackBar`，更新本文剩余数量，并运行该领域测试、
`flutter analyze` 和全量 `flutter test`。在全部调用迁移完成前，旧
`ScaffoldMessenger` 仍可能存在；新代码不得再增加裸 `SnackBar`。
