...
卡片宽度按当前字体、文案、重复计数和操作文字实际测量，短文案使用紧凑下限，长
文案在屏幕左右各保留至少 14dp 后扩展，最大 360dp。正文使用 13dp 字号，普通 tone
最多三行（`error` 放宽到六行——错误正文常含具体原因，截到三行会把关键信息藏掉），
卡片高度随换行自然增长；不得恢复固定单行省略。超过上限的信息已超出轻提示边界，
调用方应精简文案或改用 Dialog、Sheet 或完整页面。
...
- 关闭一条时其他提示不重建、不重置进度；
- 消息/计数/操作和首尾图标的布局对齐。
- 短文案保持紧凑，长文案按屏幕宽度扩展、换行且不提前省略。

真实应用（Android 真机 + 桌面 widget 测试）验证深浅主题、连续触发、手动关闭、常驻、操作、去重、优先级和跨路由
视觉。Android 真机还需检查系统安全区、手势导航和动画手感。
