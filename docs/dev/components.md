...
| `VeriRootNavigation` / `VeriRootNavigationBody` / `VeriNavigationDestination` / `veriRootPageListPadding` | Widget / 值类 / 布局 helper | `root_navigation.dart` | 四个根页面的**停靠底栏**：整宽、不透明、贴底，底色铺到屏幕最底（含系统导航条背后，否则会分成两块）。条目由自有的 [VeriBottomBar](#族-1--壳层与导航) 绘制：**未选中用 `destination.icon`（线框）、选中用 `selectedIcon`（填充）**，中文标签常显，切换时飞过的圆点用品牌色。**记账按钮不在底栏内**——由 `shell.dart` 放在 body 右下角浮动（圆角方形 `veriRadiusLg`；自绘 `Material`+`InkWell`，因为 `FloatingActionButton` 内部会吞掉长按，而长按要走 AI 记账），进出用 `AnimatedScale`+`AnimatedOpacity`。**两条约束**：① 切换动效的时间尺度由 `switchDuration`（250ms）表达：底栏那段扫过动效直接用它做时长；页面过渡由 `shell.dart` 拿同一个数值构造弹簧（`SpringDescription.withDurationAndBounce`），**没有固定时长**，只按「当前位置 + 当前速度 + 目标」平滑收敛。固定时长会让每次点击都从头重新计时，连点时页面被反复「重新起步」，看起来像没动（实测 16ms 间隔连点页面纹丝不动）；弹簧的收敛时间与距离无关，跨 1 页与跨 3 页手感一致，因此底栏与页面自然同时收住。**跨多页同样播这段过场，不瞬移**。② 安全区用 `SafeArea(minimum: 12, maintainBottomViewPadding: true)` 取 `max(系统留白, 12)`：手势提示线关闭的机器系统留白可能是 0，不给下限条目会贴边；`maintainBottomViewPadding` 让键盘弹起时底栏不跳 |
| `VeriBottomBar` / `VeriBottomBarItem` | Widget / 值类 | `veri_bottom_bar.dart` | 底栏条目绘制：切换时两条圆弧扫过、两枚圆点飞过，图标由灰渐变为品牌色并轻微摆动。**抄写自 `bottom_bar_matu` 1.5.0 并在本项目内修复，该依赖已移除**（MIT，源文件头保留版权说明）。修的五处：① 原库 `didUpdateWidget` 无条件重建全部图标 GlobalKey，父级每重建一次（切页期间两三次）就把图标的 State 整体卸载重建——图标毫无理由地重新抖一下、快速切换时动画直接断掉；现在只在条目数变化时重建 key、只在 `selectedIndex` 真变化时播动画。② 原库用同一个 `AnimationController` 交替 `forward`/`reverse` 凑出 0→1 进度（`reverse` 时返回 `1-v`），且只有上一段播完才会 `reverse`，动画中途再点会落到 `forward` 分支，按状态取值的映射一帧内从 `1-v` 跳回 `v`。③ 原库先执行 `_oldSelectedIndex = _selectedIndex`，再用 `_oldSelectedIndex < _selectedIndex` 判方向，比较恒为 false——被取消选中的图标永远按同一方向旋转。④ 原库 `await Future.delayed(200ms)` 之后才回调 `onSelect`，续体又没有 mounted/世代守卫：被后一次切换顶掉时会拿最新下标再回调一次，被跳过那次点击的回调永不送达、最终下标送达两次；现在完全由 `selectedIndex` 驱动、同步更新，`onSelect` 每次点击恰好一次，调用方不再需要 `Listener` 回声抑制。⑤ 图标旋转原式为 `-pi/(8·5v(1-v))`，分母在动画头尾趋近 0，图标会瞬间转好几圈；改为同一凸包函数直接作角度比例，中点 ±18° 不变。**注意**：切线点用 `长度 × 进度 × 1.5` 越界那处**不是**缺陷——SDK 的 `getTangentForOffset` 会把距离截断到轮廓长度，既不返回 null 也不抛异常（此前本文件记的「库的动画在 widget 测试里会崩」是误判，已更正）；本文件仍显式截断一次，以免依赖这条隐式行为 |
