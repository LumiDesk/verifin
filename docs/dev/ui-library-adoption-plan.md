...
| # | 诉求 | 候选库 | 落地可行 | 结论 |
| --- | --- | --- | --- | --- |
| 1 | 替换自研玻璃材质 | `liquid_glass_renderer` | ❌ 当前不可行 | 在本项目"Android + Flutter 3.47 + Impeller OpenGLES"组合上存在**已知未修复的 Y 轴翻转缺陷**，会渲染镜像内容。另有替代方案可选。 |
| 2 | 替换入门引导页 | `introduction_screen` | ⚠️ 不建议 | 能用，但省不下多少代码，且要为一屏引导新引入 5 个平台插件依赖；引导页真正的复杂度在业务逻辑，库覆盖不到。建议改为自研重构。 |
| 3 | 统一开关/切换条 | `animated_toggle_switch` | ✅ 可行（需自建包装层） | 动画质量高、可完全贴合设计令牌、零第三方依赖、160/160 分。但**无无障碍语义、无键盘支持**，必须包一层；纯文字分段控件不建议强行套用。 |
| 4 | 修复看板等页面布局回归 | — | ✅ 直接改（范围比预期小） | 看板并未被"改烂"：唯一的结构性改动是新增摘要卡把预算执行卡挤成了第二张（C1）。另有 1 处 2dp 文档矛盾、1 处待测的大字号疑点。详见 §7。 |
| 5 | 替换自研图表 | `syncfusion_flutter_charts` / `fl_chart` | ✅ 可行（推荐 fl_chart） | 折线/柱状/饼图可替换；**预算圆环必须保留自研**（规范强制）。Syncfusion 会破坏无障碍测试门禁，详见 §8.2–8.4。 |
| 5b | 时间图表交互 | `time_chart` | ❌ 完全不可用 | 依赖冲突导致**根本无法安装**；且它没有本项目需要的任何交互 API。 |
| 6 | 替换底部导航 | `bottom_bar_matu` | ❌ 不建议 | 声明只支持 Dart 2（`<3.0.0`）；渲染的是贴边不透明条，无法做"浮动胶囊"，与规范中两条硬性要求直接冲突。 |
| 7 | 替换自研通知 | `toastification` | ✅ 可行（需适配层） | 能替换外观与基础行为，但当前自研系统有若干**规范硬要求**（优先级队列、去重、后台暂停、常驻态），需要确认覆盖率或保留包装层。 |

一句话总结：**7 个候选里，3 个可以直接用（3、5 部分、7），1 个可修（4），2 个不建议（2、6），2 个不可用（1、5b）。**
...
| # | 决策点 | 选项 | 影响 |
| --- | --- | --- | --- |
| 1 | **图表库** | A) **fl_chart（推荐）**—MIT 无授权门槛、无无障碍缺口、四类图表全覆盖、拖动刷选与缩放平移都有 B) Syncfusion—能力最全但破坏无障碍测试门禁并有授权义务 | 决定阶段 3 的全部实现方式 |
| 2 | **玻璃方向** | A) 只调现有参数 B) 换 `glass_kit`/`blur` 放弃折射 C) 试 `liquid_glass_widgets` 承担真机验证 | 决定阶段 4 是否存在 |
| 3 | **短反馈目标** | A) 只换外观、保留全部行为契约 B) 连行为一起简化（需同步改 `docs/dev/feedback-system.md`） | 决定阶段 2 的范围与是否动规范文档 |
| 4 | **分段控件视觉** | A) 白胶囊浮在灰轨道 B) 品牌色实心 C) 语义色实心 | 决定阶段 1 的统一样式 |
| 5 | **分段控件替换范围** | 是否同意 §6.5 的划分（替换 8 处，保留 `CompactSwitchRow`/`VeriAnchoredChoice`/`FilterPill`/`MonthSwitcher`） | 决定阶段 1 的改动面 |
| 6 | **导航材质与 FAB** | 磨砂要不要关？FAB 放到哪（上浮 / 居中缺口 / 保持右侧） | 决定阶段 5 是否存在 |
| 7 | **引导页"丑"在哪** | 圆点 / 图标方块 / 配色 / 排版 | 决定是换 `smooth_page_indicator` 还是整体重构 |
| 8 | **看板卡片顺序** | A) 把新增的月度摘要卡移到预算执行卡之后（恢复"预算执行卡在看板顶部"） B) 保留现状、改 `docs/ui-guidelines.md:191` 让规范与实现一致 | 决定 M1 怎么修 |
