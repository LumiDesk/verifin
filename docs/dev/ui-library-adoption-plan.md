...
> ## ⚠️ 先读这一节：实际落地结果（2026-09-10）
>
> 本文是**评估稿**，下面的分析写于动手之前，部分结论已被实际结果取代（最典型的是
> 我当初判定 `bottom_bar_matu` 不可用，用户拍板后实际采用了它）。**判断当前实现事实
> 一律以源码、测试和 `docs/design-system.md` / `docs/dev/components.md` 为准**，不要
> 把下文的"建议/结论"当成现状。评估过程保留在此，作为决策依据的记录。
>
> | 原诉求 | 本文原结论 | **实际结果** |
> | --- | --- | --- |
> | ① 替换玻璃 | 不引入 `liquid_glass_renderer`（技术上会渲染镜像） | 用户进一步判定**玻璃方向本身是败笔**，全文玻璃实现已**整体移除**（卡片/导航/菜单/弹层改不透明实色）。见 `design-system.md`「表面材质」 |
> | ② 入门引导页 | 不建议引入 `introduction_screen` | 未引入；只对现有引导页做了视觉打磨 |
...
> | ⑥ 替换底部导航 | **不建议** `bottom_bar_matu`（认为做不出浮动形态） | **先采用、后抄写自持**。用户要求改回"贴底整宽底栏"，正是该库的形态，条目由 `BottomBarDoubleBullet` 绘制；随后实测出该库多处动画缺陷（父级每次重建都重置图标 State、进度 forward/reverse 往返断档、旋转方向判断恒为 false、`onSelect` 延迟 200ms），已抄写进 `lib/app/veri_bottom_bar.dart` 并修复，**依赖已从 pubspec 移除**。详见 `components.md` 的 `VeriBottomBar` 条目 |
...
| **8** | **移除玻璃 / 高级材质 / 背景光效** | — | ✅ **已完成** | 用户的最终判断：玻璃是设计败笔。已实施，见 §15。 |

一句话总结：**7 个候选库中，2 个可用（3、7），1 个已定（5 选 fl_chart），其余不建议/不可用；真正的重头是第 8 项——移除玻璃，已完成。**

**下一步（待用户确认后开始）**：§6 统一分段控件、§10 短反馈换外观、§8 图表替换、§5 引导页视觉重构。
...
> **执行状态（2026-09-10）**：本计划已实施完毕。提交 `refactor: remove the frosted glass material and advanced lighting`，
> 文档同步见 `docs: record the removal of the glass material across the guides`。
> `flutter analyze` 无问题，全量 `flutter test` **982 通过 / 5 跳过**，模拟器实测已无任何模糊、渐变或 Shader 层。
> 下面的计划文本保留为实施记录与后续参考。
