...
「单位：¥」「本位币 CNY」这类文字。本文件是那一轮的失效点清单与整改记录：清单经确认后已于同日随 v1.17.5 全部修复（结论见下方引用块与第 7 节），现在只作为背景与判定依据保留，**当前规则以 [设计规范](../design-system.md) 和 [组件清单](../dev/components.md) 为准**。

审查方式：六种独立搜索角度并行扫描 → 按文件分组逐条证伪 → 补漏复查。
共确认 **17 处泄漏**（含用户报告的 7 处）、**3 处元信息**（当轮判定为「单币种账本下不该出现任何币种字样」，已一并修复）、**30 余处确认干净**；另有补录的第 18 项（间距没跟单位一起收，见第 7 节）。
...
> 2026-09-10 补记：当轮判定为「单币种账本下不该出现任何币种字样」，三处已按此收掉——「货币与汇率」入口保留空串以维持宫格行高；账本列表行尾改用「账本多于一个」这个独立条件；AI 账户余额表在隐藏单位时整列去掉「币种」表头。
...
## 7. 补录：第 18 项 —— 间距没跟单位一起收（2026-09-10 追加）

前 17 处都是「文字本身漏出了单位」，这一处不同：**文字收掉了，为它单独留的间距还在**。

| 现象 | 位置 |
| --- | --- |
| 首页日历卡隐藏单位后底部空出一条 6dp：角标整块不渲染了，为它加的前置 `SizedBox(height: 6)` 还留着 | [common_widgets_panels.dart:542](../../lib/app/common_widgets_panels.dart#L542) |

成因：§6 第 2 条的收口方式是让 `MoneyUnitLabel` 自己 `return const SizedBox.shrink()`——widget 仍在树上、`find.byType` 照旧命中（这点在「连带影响」里已经预料到），但**调用方写在组件外面的间距不在组件的收口范围内**。修法是把「间距 + 角标」整块条件渲染（`if (!textCurrencyUnitHidden) ...`），实测日历卡固有高度从 399dp 降到 377dp，收掉的正是那 6dp。

同类检查结论：首页预算卡标题行、走势卡标题行、AI 结果卡的角标都没有专为单位加的前置间距（那里的间距承担的是与相邻控件的对齐），无需改动。规则已写进 [组件清单 §维护约定](../dev/components.md)。

另一处补记（同属本轮）：AI 摘要里的同币种转账现在也只报一次金额——[ai_query_tool.dart:816](../../lib/app/ai/ai_query_tool.dart#L816) 的 `to.currencyCode == fromCode` 直接返回，不再输出「→」右段，与交易列表副行的币种守卫同一口径。
