...
> | 产品 P1-8 触控目标 | 部分：`CompactSwitchRow` 整行可点、分类浮标点击区 32dp；`FilterPill` 36dp 未动（36 ≥ WCAG AA 的 24dp） | `78a7f4c` |
> | 产品 P2-5 锚点菜单行高写死 | 已修：行高按 `textScaler` 缩放 | `78a7f4c` |
> | 产品 P2-9 轻提示 | 部分：加 `liveRegion`、错误默认 8 秒、错误正文 6 行；「错误必带恢复动作」是调用点约定，未强制 | `78a7f4c` |
> | 产品 P2-10 资产页 | 已修三项：资产/负债行加 `Expanded` 防溢出、空态给「添加第一个账户」按钮、新建信用卡可直接填额度/账单日/还款日；「代还与无账户」文案已核实清楚，未改 | `78a7f4c`、`5135d75`、`bdcd635` |
> | 产品 P2-14 导入/设置可发现性 | 部分：预览标题显示目标账本、切账本给轻提示；禁用按钮原因未动 | `6473f29`、`71fb4a1` |
> | 产品 P2-15 文案与状态不符 | 已修 4 处：测试通知不再谎报已发送、补记失败专用文案、AI 设置页说明可选、AI 结果表格横向滚动 | `78a7f4c`、`6473f29` |
> | 产品 P1-11 还款页跨币种缺汇率入口 | 部分：加了说明文案与「货币与汇率」入口（可手填实际扣款金额）；页内直接编辑汇率未做 | `5135d75` |
> | 工程 8 个导入平台只有 2 个真实 fixture | 转为**接受的债 L7**（用户无法再提供样例，禁止编造格式） | `8532022` |
> | 性能 P0-1b 根页面未保活 | 已修：四个根页面保活，切回不再整页重建 | `612f178` |
> | AI 6 个查询工具 | 6 个已全部实现；`budgetStatus` 的预算聚合抽到 `lib/app/budget_status.dart`，口径仍由 controller 通过回调注入 | `d776bff`、`72a2779` |
> | 性能 P2-15 玻璃 painter 每次 paint 大量分配 | 已修：轮廓采样（Path 弧长 + 上千个切点）按尺寸/圆角缓存，拖动时只重算光照 | `32efc42` |
> | AI 结果卡标题仍是中文硬编码 | 已修：`AiToolContext` 增加 `l10n`，11 个工具的标题 / 指标名 / 表头 / 提示语全部本地化，新增 59 个 ARB 键；历史卡片保留写入时语言（既有行为） | — |
> | 性能 P1-9 高频偏好迁 ValueNotifier | **复核后不建议按原清单做**：清单里的「折叠分区」其实由资产页的页面本地状态驱动（`assets_pages.dart:69` 的 `_collapsedSections` + `setState`），控制器的折叠 API 只有测试在用；其余偏好（资产视图模式、面板配置、触感、记账动作、首页趋势）都是「改完保存才生效」的低频项。真正的大头是 113 处数据变更广播，已登记为中期债「偏好类 KV 剥离」，按原阈值触发 | — |
> | 产品 P2-14 导入预览禁用按钮无原因 | 已修：全部排除时按钮说明「没有勾选任何可导入的内容」 | `—` |
> | 产品 N-2 交易列表逐笔结余 | 已做：纯函数 `accountBalanceAfterEntry` + 设置开关「显示逐笔结余」（默认关、进备份）+ 交易行次级信息；已在模拟器验收 | `337b401` |
> | 其余 §2 条目 | 未修 | — |
>
...
| 条目 | 现状 | 证据 |
| --- | --- | --- |
| `integration_test/` 2 个用例不在任何 CI | **维持原状**：试过 Android 模拟器任务（放行 KVM 后可启动），但模拟器在 Gradle 构建期间被系统杀掉，三次调整均未通过，按用户要求撤掉，改回手动执行 | — |
